...
def gettoken(refresh_token,secret,cid):
...
def main(refresh_token,secret,cid):
    global num1
    localtime = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(int(time.time())))
    access_token=gettoken(refresh_token,secret,cid)
    headers={
...
for fo in refresh_token:
    try:
        refresh_token = fo['text']
        sid = fo['id']
        secret = fo['key']
        cid = fo['cid']
        for _ in range(0,6):
